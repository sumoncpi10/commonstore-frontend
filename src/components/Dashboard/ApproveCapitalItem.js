...
const ApproveCapitalItem = ({ notApproveCapitalItem, itemType, categroys, subcategroys, brands, models, suppliers }) => {
    ...
    useEffect(() => {
        const subCategoryNames = Array.from(new Set(notApproveCapitalItem.map(item => item?.subCategory?.subCategoryName)));
        setDistinctSubCategories(subCategoryNames);
        const CategoryNames = Array.from(new Set(notApproveCapitalItem.map(item => item?.category?.categoryName)));
        setDistinctCategories(CategoryNames);
        const zonalNames = Array.from(new Set(notApproveCapitalItem.map(item => item?.zonals?.zonalName)));
        setdistinctZonals(zonalNames);
    }, [notApproveCapitalItem]);
    //console.log(distinctCategories);
    const [dataSource, setDataSource] = useState(notApproveCapitalItem);
...
export default ApproveCapitalItem;